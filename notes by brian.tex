...
\beta\gamma(V\otimes \C^N) \otimes \beta\gamma(V^*\otimes \C^N) \cong \beta\gamma^{\otimes N^2} \otimes \beta\gamma^{\otimes N^2}
\eeqn
...
More invariantly, we can understand this system as follows.
Consider the vector space
\beqn
V \otimes \C^N \oplus V^* \otimes \C^N = \T^*(V \oplus \C^N)
\eeqn
which is equipped with the (symplectic) $SU(N)$ action given by viewing $V \otimes \C^N = V \oplus \cdots \oplus V$ as $N$ copies of the fundamental representation.
Thus, this two-dimensional theory describes the vertex algebra of chiral differential operators on the stack
\beqn
\T^*(V \otimes \C^N) \slash SU(N) = \C^{2 N^2} \slash SU(N) .
\eeqn
...
It looks like
\beqn
\beta\gamma^{\otimes 2} \otimes \beta\gamma^{\otimes N^2} \otimes bc .
\eeqn
